...
if OPENAI_API_KEY:
    http_client = httpx.Client(proxies=None, timeout=10.0)
    openai_client = OpenAI(api_key=OPENAI_API_KEY, http_client=http_client)
else:
    openai_client = None
    print("Error: OPENAI_API_KEY no está configurada. El chatbot no funcionará.")
...
def cargar_campeones():
    ...
    try:
        ...
        print(f"Excepción al cargar campeones: {str(e)}")
        return {}
...
def leer_cuentas(url):
    try:
        ...
        print(f"Error inesperado al leer las cuentas: {str(e)}")
        return []
    ...
    with cache_lock:
        ...
        for riot_id, jugador in cuentas:
            try:
                ...
                riot_id_base = riot_id.split('#')[0]
                puuid_info = obtener_puuid(api_key, riot_id_base, region)
                ...
                print(f"Error procesando {riot_id}: {str(e)}")
...
def get_players_context():
    ...
    if not datos_jugadores:
        context += "No hay datos de jugadores disponibles.\n"
    else:
...
def get_chatbot_response(user_message):
    ...
    if not openai_client:
        return "Error: El cliente de OpenAI no está configurado. Verifica OPENAI_API_KEY."
